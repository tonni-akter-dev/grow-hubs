      ...
      <div className="container px-4 grid grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-6">
        {tools.map((item, index) => (
          <motion.div
            ...
            className="bg-white cursor-pointer rounded-xl p-4 lg:p-8 flex flex-col justify-between gap-6 items-start text-left "
          >
            {/* Top content */}
            <div className="flex flex-col gap-4 lg:gap-6 w-full">
              <div className="flex justify-between w-full items-center">
                <h6 className="font-semibold leading-[35px] text-[#4E4E4E] text-h6 lg:text-h3">
                  {item.title}
                </h6>
                <Image
                  src={item.icon}
